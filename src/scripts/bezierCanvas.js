...
(function ($) {
  ...
  $.fn.bezierCanvas = function (options) {
    ...
    $(window).on("resize", function () {
      ...
      canvas.height = $(window).height() * pxlRatio;
      context.translate(
        canvas.width / 2 + settings.moveCenterX,
        canvas.height / 2 + settings.moveCenterY
      );
    });
    ...
    for (var c = 0; c < settings.maxStyles; c++) {
      styles.push({
        cR: Math.round(
          settings.colorBase.r + Math.random() * settings.colorVariation.g
        ),
        cG: Math.round(
          settings.colorBase.g + Math.random() * settings.colorVariation.g
        ),
        cB: Math.round(
          settings.colorBase.b + Math.random() * settings.colorVariation.g
        ),
    ...
    for (var k = 0; k < settings.maxLines; k++) {
      ...
      objects[color].push({
        speed: settings.speedLinear
          ? 0.1 + k / 200
          : settings.delayVariation * hm + 0.1,
        pos:
          (k * settings.lineSpacing + settings.spacingVariation * hm) *
          pxlRatio,
      });
    }
    ...
    var methods = {
      ...
      addPoints: function () {
        ...
        var m2 = p.y4 < 0 ? -1 : 1;
        var m3 =
          plusminus2 > 0 ? Math.ceil(plusminus2) : Math.floor(plusminus2);
        ...
        settings.coordinates.push({
          ...
          x4:
            -m *
            (canvas.width / 4 +
              (Math.round(Math.random() - 0.2) * canvas.height) / 4),
          y4:
            -m2 *
            m3 *
            (canvas.height / 4 +
      ...
      segmentPoints: function (
        ...
        return {
          x1:
            u0 * u0 * u0 * xP1 +
            (t0 * u0 * u0 + u0 * t0 * u0 + u0 * u0 * t0) * xP2 +
            (t0 * t0 * u0 + u0 * t0 * t0 + t0 * u0 * t0) * xP3 +
            t0 * t0 * t0 * xP4,
          x2:
            u0 * u0 * u1 * xP1 +
            (t0 * u0 * u1 + u0 * t0 * u1 + u0 * u0 * t1) * xP2 +
            (t0 * t0 * u1 + u0 * t0 * t1 + t0 * u0 * t1) * xP3 +
            t0 * t0 * t1 * xP4,
          x3:
            u0 * u1 * u1 * xP1 +
            (t0 * u1 * u1 + u0 * t1 * u1 + u0 * u1 * t1) * xP2 +
            (t0 * t1 * u1 + u0 * t1 * t1 + t0 * u1 * t1) * xP3 +
            t0 * t1 * t1 * xP4,
          x4:
            u1 * u1 * u1 * xP1 +
            (t1 * u1 * u1 + u1 * t1 * u1 + u1 * u1 * t1) * xP2 +
            (t1 * t1 * u1 + u1 * t1 * t1 + t1 * u1 * t1) * xP3 +
            t1 * t1 * t1 * xP4,
          y1:
            u0 * u0 * u0 * yP1 +
            (t0 * u0 * u0 + u0 * t0 * u0 + u0 * u0 * t0) * yP2 +
            (t0 * t0 * u0 + u0 * t0 * t0 + t0 * u0 * t0) * yP3 +
            t0 * t0 * t0 * yP4,
          y2:
            u0 * u0 * u1 * yP1 +
            (t0 * u0 * u1 + u0 * t0 * u1 + u0 * u0 * t1) * yP2 +
            (t0 * t0 * u1 + u0 * t0 * t1 + t0 * u0 * t1) * yP3 +
            t0 * t0 * t1 * yP4,
          y3:
            u0 * u1 * u1 * yP1 +
            (t0 * u1 * u1 + u0 * t1 * u1 + u0 * u1 * t1) * yP2 +
            (t0 * t1 * u1 + u0 * t1 * t1 + t0 * u1 * t1) * yP3 +
            t0 * t1 * t1 * yP4,
          y4:
            u1 * u1 * u1 * yP1 +
            (t1 * u1 * u1 + u1 * t1 * u1 + u1 * u1 * t1) * yP2 +
            (t1 * t1 * u1 + u1 * t1 * t1 + t1 * u1 * t1) * yP3 +
            t1 * t1 * t1 * yP4,
        };
      },
    };
    ...
    function draw() {
      ...
      for (var c = 0; c < styles.length; c++) {
        ...
        for (var j = 0; j < objects[c].length; j++) {
          ...
          if (settings.loop) var cl = l % (settings.coordinates.length - 1);
          else var cl = l;
          ...
          if (settings.spacingMode == "multiply") {
            ...
            var p = methods.segmentPoints(
              ...
              Math.min(t0, 1),
              Math.min(t1, 1),
              settings.coordinates[cl].x1 +
                o.pos * settings.coordinates[cl].kx1,
              settings.coordinates[cl].y1 +
                o.pos * settings.coordinates[cl].ky1,
              settings.coordinates[cl].x2 +
                o.pos * settings.coordinates[cl].kx1,
              settings.coordinates[cl].y2 +
                o.pos * settings.coordinates[cl].ky1,
              settings.coordinates[cl].x3 +
                o.pos * settings.coordinates[cl].kx2,
              settings.coordinates[cl].y3 +
                o.pos * settings.coordinates[cl].ky2,
              settings.coordinates[cl].x4 +
                o.pos * settings.coordinates[cl].kx2,
          ...
          context.bezierCurveTo(p.x2, p.y2, p.x3, p.y3, p.x4, p.y4);
          ...
          if (gt1 - l >= 1) {
            // If looped, then max value is loop end
            l = l + 1;
            if (settings.loop) cl = l % (settings.coordinates.length - 1);
            else cl = l;
            ...
            context.bezierCurveTo(pp.x2, pp.y2, pp.x3, pp.y3, pp.x4, pp.y4);
          }
        }
